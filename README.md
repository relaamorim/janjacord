...
- **Som só do jogo, sem configurar nada**: ao compartilhar, marque **"Compartilhar também o som"** e deixe **"Só do aplicativo escolhido"** — o JanjaCord captura apenas o áudio daquele programa (nada de notificação do WhatsApp nem das vozes da sala indo para a transmissão). Se você compartilhar a tela inteira, ele pergunta de qual aplicativo vem o som. Isso é feito por um pequeno programa nativo nosso (pasta `nativo/`), com a mesma API do Windows que o Discord e o OBS usam; requer Windows 10 versão 2004 ou mais novo. Desde a 1.13 a captura é feita em **todos os canais do dispositivo** e reduzida para estéreo pelo próprio JanjaCord — em placas e headsets **7.1** isso evita o som chegar ~13 dB mais baixo (um fator de normalização que o Windows aplica quando se pede a captura já em estéreo) — e o volume do aplicativo no **Mixer de volume do Windows** é compensado, para a transmissão sair no nível real do jogo, e não no volume local de quem transmite. Se o aplicativo estiver mudo no Mixer, o JanjaCord avisa. As opções **"todo o computador"** e **"dispositivo de áudio"** (para quem já usa cabo virtual) continuam disponíveis.
...
- **Som de um aplicativo** — `nativo/janjacord-audio.cs` é um programinha em C# (compilado com o `csc.exe` que já vem no Windows: `npm run compilar-audio`) que usa a API *process loopback* do Windows para capturar o áudio de um único processo e entregá-lo ao JanjaCord, que o coloca na transmissão. O executável fica em `bin/` e vai fora do pacote do app (`asarUnpack`) para poder ser executado. Ele captura no formato de mixagem do dispositivo (ex.: 8 canais float em headsets 7.1), faz a redução para estéreo com coeficientes clássicos (sem o fator de normalização do Windows), compensa o volume da sessão do aplicativo no Mixer (até +20 dB) e entrega PCM 48 kHz, estéreo, 16 bits. Na interface, esse áudio entra num `AudioWorklet` com fila — que não depende dos temporizadores da página, desacelerados pelo Chromium quando a janela do JanjaCord está coberta pelo jogo — e a trilha resultante segue junto com o vídeo para os espectadores.
