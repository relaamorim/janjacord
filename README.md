...
- **Som de um aplicativo** — `nativo/janjacord-audio.cs` é um programinha em C# (compilado com o `csc.exe` que já vem no Windows: `npm run compilar-audio`) que usa a API *process loopback* do Windows para capturar o áudio de um único processo e entregá-lo ao JanjaCord, que o coloca na transmissão. O executável fica em `bin/` e vai fora do pacote do app (`asarUnpack`) para poder ser executado.
